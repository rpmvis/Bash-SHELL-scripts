#!/bin/bash
# script for setting Linux directory / file permissions with Symfony framework.
# Permissions are set for the "console", "", " , amongst others.
...
if [ $# > 0 ]; then
  case $1 in
	...
	permissions)
	    ...
	    sessions_dir_options="./var/sessions;.app/sessions"
	    get_dir_existent $sessions_dir_options sessions_dir
        if [[ ! $cache_dir ]]; then
        ...
        msg="${PROGNAME}: Permissions set for $me:$linux_group_name via setfacl in directories:\n"
        msg+="'$cache_dir'\n"
        msg+="'$logs_dir'\n"

        if [[ $sessions_dir ]]; then
           sudo chown -R "$me":"$linux_group_name" $sessions_dir
           sudo setfacl -R -m u:"$me":rwX -m u:"$linux_group_name":rwX $sessions_dir
           msg+="'$sessions_dir'\n"
        fi
        echo -e $msg # -e for newline
    ;;
